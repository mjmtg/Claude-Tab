...
Codex Tabs is a **Tauri 2 desktop application** — a tab-based terminal manager with an extension-based plugin system. Frontend is React/TypeScript with xterm.js; backend is Rust with Tokio.
...
- **`storage`** — SQLite persistence (`~/.Codex-tabs/archive.db`)
- **`tauri-bridge`** — Tauri IPC commands and event forwarding
- **`extensions/`** — `Codex-hooks`, `auto-switch`, `output-parser`, `file-watcher`, `system-notify`
